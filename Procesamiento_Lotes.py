...
# Simulación de procesamiento por lotes con múltiples operaciones aritméticas
...
def mostrar_estado(lotes, lote_actual, procesos_terminados, tiempo_global):
    ...
    if lote_actual < len(lotes):
        print(f"\nLote en Ejecución (Lote {lote_actual + 1}):")
        for proceso in lotes[lote_actual]:
            print(
                f" - Programa {proceso['NumeroPrograma']}: "
                f"Tiempo Máximo Estimado {proceso['Tiempo']}s"
            )

    print(f"\nProcesos Terminados:")
    for terminado in procesos_terminados:
        print(
            f" - Programa {terminado['NumeroPrograma']}: "
            f"{terminado['Operacion']} {terminado['a']} {terminado['Operacion']} {terminado['b']} = {terminado['Resultado']}"
        )
